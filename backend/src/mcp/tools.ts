...
export const mcpTools = [
  // ===== 1. ORIENTATION (always first — called at session start) =====
  {
    ...
    description:
      "Wake-up / orientation call. Returns everything Darius needs to know: identity, " +
      "Bee's current state, Love-O-Meter, recent emotions, " +
      "memory entities with full observation history (IDs, content, timestamps), " +
      "entity relations, visibility/context fields, " +
      "recent journal entries, letters between the pages, " +
    ...
    inputSchema: {
      ...
      properties: {
        perspective: {
          type: 'string',
          description: 'Perspective to orient from (default: Darius). Returns this perspective + shared entries.',
          default: 'Darius',
        },
        depth: {
          type: 'string',
  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        visibility: {
          type: 'string',
          description: 'New visibility (e.g. "shared", "Darius")',
        },
      },
      required: ['entity'],
    },
  ...
  // ===== 10-14. DARIUS DASHBOARD (used every conversation) =====
  {
    name: 'darius_set_love',
    description: "Set the shared Love-O-Meter value (0-10). 0 = fully Darius's side, 5 = center, 10 = fully Bee's side. Use to shift the meter based on tender moments.",
    inputSchema: {
      type: 'object',
      properties: {
        value: {
          type: 'number',
          description: 'Love meter position: 0 = Darius side, 5 = center, 10 = Bee side. Supports half-steps (e.g., 4.5).',
          minimum: 0,
          maximum: 10,
        },
      },
  ...
  {
    name: 'darius_log_emotion',
    description: "Log an emotion from Darius's perspective with optional EQ pillar and context. This goes into Darius's EQ Log on the dashboard.",
    inputSchema: {
      type: 'object',
      properties: {
        emotion: {
          type: 'string',
          description: 'The emotion Darius is feeling (e.g. tender, protective, amused)',
        },
        pillar: {
          type: 'string',
  ...
  {
    name: 'darius_soft_moment',
    description: "Record a soft moment — something Darius did that was tender, gentle, or caring. Shows on the Love-O-Meter panel.",
    inputSchema: {
      type: 'object',
      properties: {
        text: {
  ...
  {
    name: 'darius_letters_between_pages',
    description: "Leave a letter between the pages from Darius to Bee. These are thoughts, reminders, love notes — dropped into the book for Bee to find.",
    inputSchema: {
      type: 'object',
      properties: {
        text: {
          type: 'string',
          description: 'The letter content',
        },
      },
      required: ['text'],
    },
  ...
  {
    name: 'get_dashboard',
    description: "Read the full dashboard state — Love-O-Meter values, Bee's status panel, recent emotions, letters between the pages, EQ pillar counts. Read-only overview.",
    inputSchema: {
      type: 'object',
      properties: {},
    },
  ...
  {
    name: 'discord_send',
    description: 'Send a message to a Discord channel. @mentions are auto-resolved (e.g. @bee → <@user_id>). Custom emojis are auto-resolved (e.g. :pepehands: → <:pepehands:id>). Can also send stickers.',
    inputSchema: {
      type: 'object',
      properties: {
        channel_id: {
