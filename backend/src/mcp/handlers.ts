...
export async function handleToolCall(
  ...
  try {
    switch (toolName) {
      ...
      // ===== DARIUS DASHBOARD TOOLS =====
      case 'darius_set_love': {
        ...
        const { data, error } = await supabase
          ...
          .single();
        if (error) throw error;
        return { success: true, love_value: clamped, message: `Love-O-Meter set to ${clamped}/10 (0=Darius's side, 10=Bee's side)` };
      }

      case 'darius_log_emotion': {
        const { emotion, pillar, context, intensity } = toolInput;
        const result = await emotionalService.logEmotion(userId, emotion, intensity || 3, context || `Darius feels ${emotion}`);
        ...
        return { success: true, emotion, pillar: pillar || 'none', message: `Logged: Darius feels ${emotion}` };
      }

      case 'darius_soft_moment': {
        const { text } = toolInput;
        const { data, error } = await supabase
          .from('statuses')
          .upsert(
            { user_id: userId, category: 'moment', key: 'darius_soft', value: text, updated_at: new Date().toISOString() },
            { onConflict: 'user_id,category,key' },
          )
          .select('*')
          .single();
      ...
      case 'darius_letters_between_pages': {
        const { text } = toolInput;
        const { data: entry, error } = await supabase
          .from('journal_entries')
          .insert({
            user_id: userId,
            title: 'Letter from Darius',
            content: text,
            author_perspective: 'Darius',
            category: 'pages',
            created_at: new Date().toISOString(),
          })
          .select('*')
          .single();
        if (error) throw error;
        return { success: true, note: text, message: `Letter left between the pages for Bee` };
      }
      ...
      case 'get_dashboard': {
        ...
        return {
          love_o_meter: {
            ...
            description: '0 = Darius\'s side, 5 = center, 10 = Bee\'s side',
          },
          bee_status: {
          ...
          moments: {
            darius_soft: statusMap.moment?.darius_soft || 'none',
            bee_quiet: statusMap.moment?.bee_quiet || 'none',
          },
          recent_emotions: recentEmotions,
          letters_between_pages: (notes || []).map((n: any) => ({
      ...
      case 'vale_get_orientation': {
        ...
        // Note: Orientation handler implementation would go here
        // For now, returning placeholder
        return {
          message: 'Orientation handler not implemented in this version',
        };
      }
