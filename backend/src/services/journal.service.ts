...
export async function createJournalEntry(userId: string, input: CreateJournalInput) {
  ...
  if (error) throw new AppError(500, `Failed to create journal entry: ${error.message}`);
  return data;
}
...
export async function listJournalEntries(
  ...
  if (error) throw new AppError(500, `Failed to list journal entries: ${error.message}`);
...
export async function getJournalEntry(userId: string, entryId: string) {
  ...
  if (error || !data) throw new AppError(404, 'Journal entry not found');
  return data;
}
...
export async function updateJournalEntry(
  ...
  if (error) throw new AppError(500, `Failed to update journal entry: ${error.message}`);
  return data;
}
...
export async function deleteJournalEntry(userId: string, entryId: string) {
  ...
  if (error) throw new AppError(500, `Failed to delete journal entry: ${error.message}`);
  return { success: true };
}
...
export async function setStatus(userId: string, category: string, key: string, value: string) {
  ...
  if (error) throw new AppError(500, `Failed to set status: ${error.message}`);
  return data;
}
...
export async function getStatus(userId: string, category?: string) {
  ...
  if (error) throw new AppError(500, `Failed to get status: ${error.message}`);
  return data || [];
}
...
export async function deleteStatus(userId: string, category: string, key: string) {
  ...
  if (error) throw new AppError(500, `Failed to delete status: ${error.message}`);
  return { success: true };
}
...
export async function setIdentity(
  ...
  if (error) throw new AppError(500, `Failed to set identity: ${error.message}`);
  return data;
}
...
export async function getIdentity(userId: string, perspective?: string) {
  ...
  const { data, error } = await query;
  if (error) throw new AppError(500, `Failed to get identity: ${error.message}`);
  return data || [];
}
...
export async function deleteIdentity(userId: string, perspective: string, category: string, key: string) {
  ...
  if (error) throw new AppError(500, `Failed to delete identity: ${error.message}`);
  return { success: true };
}
